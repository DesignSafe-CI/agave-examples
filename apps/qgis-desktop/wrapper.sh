# VNC/QGIS Desktop wrapper for DesignSafe on a Virtual Machine in a container
...
docker run -i --sig-proxy=true --rm \
  ...
  -v /corral-repl/tacc/NHERI/community:/home/ubuntu/community:ro \
  --env VNCP="$VNCP" --env DESKTOP_RESOLUTION="${desktop_resolution}" \
  stevemock/designsafe-vnc-qgis
...
if [ ! $? ]; then
        echo "QGIS Desktop exited with an error status. $?" >&2
