...
## setting up VNC password on the fly
VNCP='${AGAVE_JOB_ID}'
VNCPO=`/work/00849/tg458981/maverick/vncp $VNCP`
...
WEBSOCKIFY_OUT=`/home/00832/envision/websockify/run --cert=/home/00832/envision/.viscert/vis.2015.04.pem -D 5902 localhost:$LOCAL_VNC_PORT`
echo $WEBSOCKIFY_OUT

#  queues have row-node numbering, put login port in 50K - 51K block to avoid collisions with stampede ports
LOGIN_VNC_PORT="$((49+$VNC_DISPLAY))`echo $NODE_HOSTNAME | perl -ne 'print $1.$2.$3 if /c\d\d(\d)-(\d)\d(\d)/;'`"
...
# maverick specific port offset + 1,000 for websocket port from websockify
WEBSOCKET_PORT=$(($LOGIN_VNC_PORT + 1000))
